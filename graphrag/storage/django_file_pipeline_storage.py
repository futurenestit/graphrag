...
class DjangoFilePipelineStorage(PipelineStorage):
    ...
    async def set(self, key: str, value: Any, encoding: str | None = None) -> None:
        ...
        content = ContentFile(value) if is_bytes else ContentFile(value.encode(encoding or self._encoding))
        default_storage.save(join_path(self._root_dir, key), content)

    async def has(self, key: str) -> bool:
        """Has method definition."""
        return default_storage.exists(join_path(self._root_dir, key))

    async def delete(self, key: str) -> None:
        """Delete method definition."""
        if await self.has(key):
            default_storage.delete(join_path(self._root_dir, key))
    ...
    def child(self, name: str | None) -> "DjangoFilePipelineStorage":
        """Create a child storage instance."""
        if name is None:
            return self
        return DjangoFilePipelineStorage(str(Path(self._root_dir) / Path(name)))
...
def join_path(file_path: str, file_name: str) -> str:
...
def create_django_file_storage(**kwargs: Any) -> PipelineStorage:
    ...
    return DjangoFilePipelineStorage(root_dir=base_dir)
